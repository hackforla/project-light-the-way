...
describe('Dire CRUD tests', function () {
      ...
      .end(function (signinErr, signinRes) {
        ...
        agent.post('/api/dires')
          ...
          .end(function (direSaveErr, direSaveRes) {
            ...
            // Get a list of Dires
            agent.get('/api/dires')
              .end(function (diresGetErr, diresGetRes) {
                // Handle Dires save error
                if (diresGetErr) {
                  return done(diresGetErr);
                }

                // Get Dires list
                var dires = diresGetRes.body;

                // Set assertions
                (dires[0].user._id).should.equal(userId);
                (dires[0].name).should.match('Dire name');
      ...
      .end(function (signinErr, signinRes) {
        ...
        agent.post('/api/dires')
          ...
          .end(function (direSaveErr, direSaveRes) {
            ...
            agent.put('/api/dires/' + direSaveRes.body._id)
              .send(dire)
              .expect(200)
  ...
  it('should be able to get a list of Dires if not signed in', function (done) {
  ...
  it('should be able to get a single Dire if not signed in', function (done) {
    ...
    // Save the Dire
    direObj.save(function () {
      request(app).get('/api/dires/' + direObj._id)
        .end(function (req, res) {
          // Set assertion
  ...
  it('should return proper error for single Dire with an invalid Id, if not signed in', function (done) {
    ...
    request(app).get('/api/dires/test')
      .end(function (req, res) {
        // Set assertion
  ...
  it('should return proper error for single Dire which doesnt exist, if not signed in', function (done) {
    ...
    request(app).get('/api/dires/559e9cd815f80b4c256a8f41')
      .end(function (req, res) {
        // Set assertion
      ...
      .end(function (signinErr, signinRes) {
        ...
        agent.post('/api/dires')
          ...
          .end(function (direSaveErr, direSaveRes) {
            ...
            agent.delete('/api/dires/' + direSaveRes.body._id)
              .send(dire)
              .expect(200)
  ...
  it('should not be able to delete an Dire if not signed in', function (done) {
    ...
    direObj.save(function () {
      // Try deleting Dire
      request(app).delete('/api/dires/' + direObj._id)
        .expect(403)
        .end(function (direDeleteErr, direDeleteRes) {
          // Set message assertion
              ...
              orphan.remove(function () {
                ...
                agent.post('/api/auth/signin')
                  ...
                  .end(function (err, res) {
                    ...
                    // Get the Dire
                    agent.get('/api/dires/' + direSaveRes.body._id)
                      .expect(200)
